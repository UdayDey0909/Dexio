...
const PokemonCardImage: React.FC<PokemonCardImageProps> = ({
   ...
   // Simplified pokeball colors based on primary type
   const pokeballColors = useMemo(() => {
      if (types && types.length > 0) {
         const primaryTypeColor = getTypeColor(types[0]);
         const lightenedColor = lightenColor(primaryTypeColor, 30); // Slightly lighten the type color

         return {
            topColor: lightenedColor,
            bottomColor: lightenedColor,
            middleColor: lightenColor(primaryTypeColor, 50), // More lightened for the center
            bandColor: primaryTypeColor, // Keep the original type color for the band
         };
      }
