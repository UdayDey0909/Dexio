...
export const usePokedexEntries = (
   ...
   // Fetch function
   const fetchPokedexEntries = useCallback(async (pokeName: string) => {
      updatePokedexEntriesState(setState, { loading: true, error: null });

      try {
         const entries = await gameService.getPokedexEntries(pokeName);
         updatePokedexEntriesState(setState, { data: entries, loading: false });
      } catch (error) {
